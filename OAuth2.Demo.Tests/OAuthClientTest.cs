...
namespace OAuth2.Demo.Tests
{
    public class OAuthClientTest
    {
        ...
        private static async Task<string> GetAuthorizationCode()
        {
            ...
            string redrect_uri=HttpUtility.UrlEncode("http://localhost:8001/api/authorization_code");

            var response = await _httpClient.GetAsync(
                "/authorize?grant_type=authorization_code&response_type=code&client_id="+clientId+"&redirect_uri="+redrect_uri);
        ...
        public async Task OAuth_Implicit_Test()
        {
            ...
            string redirect_uri = HttpUtility.UrlEncode("http://localhost:8001/api/access_token");

            var tokenResponse = await _httpClient.GetAsync(
                "/authorize?response_type=token&client_id=" + clientId + "&redirect_uri=" + redirect_uri);
